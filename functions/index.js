// Ported from PDFSign's functions/detectFields (vcarmel-cell/pdfsign) — same
// prompt/schema/model, adapted only in authorization (HR-Manager has no
// premium-tier system, just superadmin) and allowed origins.
...
const OWNER_EMAIL = 'v.carmel@gmail.com';
const ALLOWED_ORIGINS = ['https://vcarmel-cell.github.io', 'https://hr.next1.co.il'];
// gemini-flash-latest is an alias Google keeps pointed at their current
// "flash" model - chosen deliberately over a pinned model name (e.g.
// gemini-2.5-flash), since pinned names get deprecated over time (hit this
// exact staleness problem once already, in PDFSign).
const GEMINI_MODEL = 'gemini-flash-latest';

const FIELD_TYPES = ['text', 'number', 'idNumber', 'phone', 'date', 'checkbox', 'signature'];
...
- type: infer from that label text -
  ...
  Israeli ID number label (e.g. "ת.ז.", "מספר זהות", "תעודת זהות") -> "idNumber";
...
exports.detectFields = onRequest(
  ...
  async (req, res) => {
    ...
    let authorized = decoded.email === OWNER_EMAIL;
    if (!authorized) {
      try {
        const snap = await admin.firestore().collection('users').doc(decoded.uid).get();
        const data = snap.exists ? snap.data() : null;
        authorized = !!data && data.active === true && data.role === 'superadmin';
      } catch (err) {
